...
const RecentMovies = (function() {
  let currentPage = 1;
  const apiKey = '724fa79eddb78094bf31242df4a56d93';
  const youtubeApiKey = "AIzaSyAYjh_9bTwrrncv2ENpEGSYKHtgJjv4QJY";
  const apiUrl = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=en&sort_by=popularity.desc&page=9`;
  const maxMoviesPerPage = 19;
  let moviesList;
  ...
  function displayMovies() {
    ...
    for (let i = 0; i < Math.min(moviesList.length, maxMoviesPerPage); i++) {
      container += `
            <div class="div-card w-100">  
              <a href="pageMove.html" class="btn-recent" data-recent-index="${i}">
    ...
    document.getElementById("releaseDate").innerHTML = container;
    attachTrailerEvents();
  }
  ...
  function attachTrailerEvents() {
    const films = document.querySelectorAll(".btn-recent");
    films.forEach((film) => {
      film.addEventListener("click", function (event) {
        event.preventDefault();
        const index = this.getAttribute("data-recent-index");
  ...
  document.getElementById('nextreleaseDateButton').addEventListener('click', function () {
    currentPage++;
